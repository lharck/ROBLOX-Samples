...
function PunchDummy:MakeButtonGreen(button)
	...
	if self.Clicked then
		...
		play_punch(character, button.Name.."_Hit", speed_amount, button, self.Model)	
	end
...
-- inserts animations for player to use during the exercise
function PunchDummy:InsertAnimations()
	local character = self.Player.Character
	
	local animData = {
		{"Top_Hit", "1358217702"},
		{"Bottom_Hit", "1358216498"},
		{"Left_Hit", "1358213364"},
		{"Right_Hit", "1358214487"},
		{"Idle", "1348669282"},
	}
	
	local anims = {}
	for _, data in animData do
		local anim = Instance.new("Animation", character)
		anim.Name = data[1]
		anim.AnimationId = "http://www.roblox.com/Asset?ID=" .. data[2]
		anims[#anims+1] = anim
	end
...
function PunchDummy:PrepareForStart()
	...
	self.animations = {self:InsertAnimations()}

	glove_module.WearGloves(player.Character)

	character:FindFirstChild("HumanoidRootPart").CFrame = 
		CFrame.new(self.Model.Torso_Position.Position)
		* CFrame.Angles(
			math.rad(self.Model.Torso_Position.Orientation.X), 
			math.rad(self.Model.Torso_Position.Orientation.Y), 
			math.rad(self.Model.Torso_Position.Orientation.Z)
		)

	local idleAnimation = character.Humanoid:LoadAnimation(character.Idle)
	idleAnimation.Priority = 2
	idleAnimation.Looped = true
	idleAnimation:Play()
	idleAnimation:AdjustSpeed(1)
end
...
function play_punch(character, name, speed, button, dummy_model)
	...
	local Animation = humanoid:LoadAnimation(character:FindFirstChild(name))
